...
# Preserves budget for the final-2-day full burst. Usage: ./cheap.sh [target] [bleed_cap] [leg_usd] [cost_ceil_per_1k]
cd ~/dreamdex-r3/backend
TARGET=${1:-400000}; BLEED=${2:-40}; LEG=${3:-50}; COST_CEIL=${4:-0.15}
exec docker compose run --rm --no-deps -T \
  ...
  -e CLIMB_SPREAD_GATE_PCT=0.05 -e CLIMB_COST_CEIL_PER_1K=$COST_CEIL -e CLIMB_PAUSE_EXP_S=45 -e CLIMB_COST_WINDOW=15 \
