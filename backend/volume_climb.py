...
def snap_lot(sp, q):
    n = max(round(q / sp["lot"]), 1); q = round(n * sp["lot"], 10)
    return q if q >= sp["minq"] else round(sp["minq"], 10)
def snap_price(sp, p):
    t = sp["tick"]; return round(round(p / t) * t, 10)
...
def pick_cheapest():
    """Return (spec, ob, spread_pct) for the tightest-spread pair that clears the
    gate, or (None, None, best_spread) if none qualify."""
    best = None
    for sp in SPECS.values():
        try:
            ob = dex.get_orderbook(sp["pair"])
    ...
    if SPREAD_GATE_PCT > 0 and spread_pct > SPREAD_GATE_PCT:
        return None, None, spread_pct   # even the cheapest is too wide
    return sp, ob, spread_pct
...
for i in range(MAX_ITERS):
    try:
        ...
        sp, ob, spread_pct = pick_cheapest()
        if sp is None:
            wide = f"{spread_pct:.3f}%" if spread_pct is not None else "no book"
            if not _paused: tg(f"⏸ Paused @ ${vol:,.0f} vol — cheapest spread {wide} > gate {SPREAD_GATE_PCT}%"); _paused = True
            print(f"[{i}] cheapest spread {wide} > gate {SPREAD_GATE_PCT}% — pause {PAUSE_EXP_S:.0f}s")
            time.sleep(PAUSE_EXP_S); continue

        if bb(sp) > sp["minq"]:
            stop(f"unexpected base inventory on {sp['pair']} {bb(sp):.6f}")
        mid = (ob["bid"]+ob["ask"])/2
        qty = snap_lot(sp, LEG_USD/mid)
        buy_lim  = snap_price(sp, ob["ask"]*(1+SLIP_PCT))
        sell_lim = snap_price(sp, ob["bid"]*(1-SLIP_PCT))
        tag = sp["pair"].split(":")[0]
        ...
        dex.place_order(sp["pair"],"sell",snap_lot(sp, got),order_type="ioc",limit_price=sell_lim,funding="wallet")
